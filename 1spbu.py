...
def process_url(line):
    ...
    try:
        ...
        if response.status_code == 200:
            with lock:
                successful_urls.append(url)  # 将成功的URL添加到列表中
                print(url)
    except Exception as e:  # 打印具体的异常信息
        print(f"连接 {url} 失败: {e}")

def save_to_csv(urls):
    with open('successful_urls.csv', 'w', newline='', encoding='utf-8') as file:
